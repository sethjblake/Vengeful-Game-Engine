...
std::shared_ptr<luabridge::LuaRef> VengefulGameEngine::createComponentOfType(std::string type, int actorID, std::string key)
{
	...
	(*tablePointer)["uniqueComponentID"] = uniqueComponentID;

	if (type == "Rigidbody") {

		if (!globalPhysicsSystem.worldExists()) {
			globalPhysicsSystem.createWorld();
		}

		Rigidbody* rigidbody = globalPhysicsSystem.createRigidbody(uniqueComponentID);
		(*tablePointer)["Rigidbody"] = rigidbody;
		return tablePointer;
	}

	if (type == "ParticleSystem") {
		ParticleSystem* particleSystem = globalParticleSystemsManager.createParticleSystem(uniqueComponentID, actorID, key);
...
VengefulGameEngine::Actor* VengefulGameEngine::CreateActorFromDocumentArray(rapidjson::Document& document, int index)
{
	...
	if (arr[index].HasMember(key.c_str())) {
		...
		actor = CreateActorFromTemplate(templateName);
	}
	else {
...
VengefulGameEngine::Actor* VengefulGameEngine::CreateActorFromTemplate(std::string actorTemplateName)
{
...
VengefulGameEngine::Actor* VengefulGameEngine::CreateEmptyActor()
{
...
VengefulGameEngine::Actor* VengefulGameEngine::Instantiate(std::string actorTemplateName) {
	Actor* actor = CreateActorFromTemplate(actorTemplateName);
