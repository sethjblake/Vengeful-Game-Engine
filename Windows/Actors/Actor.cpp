...
void VengefulGameEngine::Actor::injectLuaReferences(std::shared_ptr<luabridge::LuaRef> componentRef)
{
    ...
    luabridge::LuaRef rigidbodyRef = (*componentRef)["Rigidbody"];

    if (!rigidbodyRef.isNil()) {
        rigidbodyRef["actor"] = this;
    }

    luabridge::LuaRef particleSystemRef = (*componentRef)["ParticleSystem"];
...
luabridge::LuaRef VengefulGameEngine::Actor::GetComponentByKey(std::string key)
{
    ...
    luabridge::LuaRef componentRef = *components[key]->luaRef;

    if (componentRef["enabled"] == false) {
        return luabridge::LuaRef(luaState);
    }
...
luabridge::LuaRef VengefulGameEngine::Actor::GetComponents(std::string typeName)
{
    ...
    for (auto& component : components) {
        if (component.second->type == typeName) {
            ...
            if (luaRef["enabled"] == false) continue;

            luabridge::LuaRef rigidbodyRef = luaRef["Rigidbody"];

            if (!rigidbodyRef.isNil()) {
                if (rigidbodyRef["enabled"] == true) componentsTable[luaIndex++] = rigidbodyRef;
                continue;
            }

            luabridge::LuaRef particleSystemRef = luaRef["ParticleSystem"];

            if (!particleSystemRef.isNil()) {
                if (particleSystemRef["enabled"] == true) componentsTable[luaIndex++] = particleSystemRef;
                continue;
            }
...
luabridge::LuaRef VengefulGameEngine::Actor::AddComponent(std::string typeName)
{
    ...
    int uniqueComponentID = globalComponentCounter++;

    newTable["uniqueComponentID"] = uniqueComponentID;

    if (typeName == "Rigidbody") {
        newTable["Rigidbody"] = globalPhysicsSystem.createRigidbody(uniqueComponentID);
    }
    else if (typeName == "ParticleSystem") {
        newTable["ParticleSystem"] = globalParticleSystemsManager.createParticleSystem(uniqueComponentID, uniqueActorID, key);
    }
    else {
