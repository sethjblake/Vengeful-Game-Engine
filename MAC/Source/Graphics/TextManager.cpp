...
void VengefulGameEngine::TextManager::generateTextures(SDL_Renderer* renderer, VengefulGameEngine::ContentLoader& contentLoader)
{
	while (!globalTextRequests.empty()) {
		...
		uint8_t r = static_cast<uint8_t>(textRequest.r);
		uint8_t g = static_cast<uint8_t>(textRequest.g);
		uint8_t b = static_cast<uint8_t>(textRequest.b);
		uint8_t alpha = static_cast<uint8_t>(textRequest.a);
