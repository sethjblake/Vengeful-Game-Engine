...
#if defined(STOP_ON_DESIGNATED_FRAME)

	if (Helper::GetFrameNumber() >= DESIGNATED_FRAME) {
		std::cout << ""; // place breakpoint here
	}
...
{
	if (!globalPhysicsSystem.newRigidbodyIDs.empty()) {
		std::vector<int>& rigidbodyIDs = globalPhysicsSystem.newRigidbodyIDs;
		for (int i = 0; i < rigidbodyIDs.size(); i++) {
			Rigidbody* rigidbody = globalPhysicsSystem.rigidbodies[rigidbodyIDs[i]];
			rigidbody->OnStart();
		}
		globalPhysicsSystem.newRigidbodyIDs.clear();
	}
