...
public:
	...
	Actor(std::string actor_name, char view, glm::ivec2 position, glm::ivec2 initial_velocity,
		...
		: actor_name(actor_name), view(view), position(position), velocity(initial_velocity), blocking(blocking), nearbyDialogue(nearbyDialogue), contactDialogue(contactDialogue) {
	}
...
std::vector<Actor> hardcoded_actors = {
	Actor(
		"spartan1", // Actor name
		's', // view
