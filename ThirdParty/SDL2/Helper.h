	...
	static void SDL_RenderPresent(SDL_Renderer* renderer)
	{
		...
		if (RECORDING_MODE || _autograder_mode)
		{
			if (!initialized)
			{
				...
				int colliderHeight, colliderWidth;
				SDL_GetRendererOutputSize(renderer, &colliderWidth, &colliderHeight);
				saving_surface = SDL_CreateRGBSurfaceWithFormat(0, colliderWidth, colliderHeight, 24, SDL_PIXELFORMAT_RGB24);
